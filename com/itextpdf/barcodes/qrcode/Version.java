...
/*     */ final class Version
/*     */ {
/*  58 */   private static final int[] VERSION_DECODE_INFO = new int[] { 31892, 34236, 39577, 42195, 48118, 51042, 55367, 58893, 63784, 68472, 70749, 76311, 79154, 84390, 87683, 92361, 96236, 102084, 102881, 110507, 110734, 117786, 119615, 126325, 127568, 133589, 136944, 141498, 145311, 150283, 152622, 158308, 161089, 167017 };
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */   
/*  68 */   private static final Version[] VERSIONS = buildVersions();
/*     */ 
/*     */   
...
/*  81 */     this.versionNumber = versionNumber;
/*  82 */     this.alignmentPatternCenters = (int[])alignmentPatternCenters.clone();
/*  83 */     this.ecBlocks = new ECBlocks[] { ecBlocks1, ecBlocks2, ecBlocks3, ecBlocks4 };
/*  84 */     int total = 0;
/*  85 */     int ecCodewords = ecBlocks1.getECCodewordsPerBlock();
/*  86 */     ECB[] ecbArray = ecBlocks1.getECBlocks();
/*  87 */     for (int i = 0; i < ecbArray.length; i++) {
/*  88 */       ECB ecBlock = ecbArray[i];
/*  89 */       total += ecBlock.getCount() * (ecBlock.getDataCodewords() + ecCodewords);
/*     */     } 
/*  91 */     this.totalCodewords = total;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 105 */     return this.alignmentPatternCenters;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 112 */     return this.totalCodewords;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 120 */     return 17 + 4 * this.versionNumber;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 128 */     return this.ecBlocks[ecLevel.ordinal()];
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 138 */     if (dimension % 4 != 1) {
/* 139 */       throw new IllegalArgumentException();
/*     */     }
/*     */     try {
/* 142 */       return getVersionForNumber(dimension - 17 >> 2);
...
/* 153 */     if (versionNumber < 1 || versionNumber > 40) {
/* 154 */       throw new IllegalArgumentException();
/*     */     }
/* 156 */     return VERSIONS[versionNumber - 1];
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 165 */     int bestDifference = Integer.MAX_VALUE;
/* 166 */     int bestVersion = 0;
/* 167 */     for (int i = 0; i < VERSION_DECODE_INFO.length; i++) {
/* 168 */       int targetVersion = VERSION_DECODE_INFO[i];
/*     */       
/* 170 */       if (targetVersion == versionBits) {
/* 171 */         return getVersionForNumber(i + 7);
/*     */       }
/*     */ 
/*     */       
/* 175 */       int bitsDifference = FormatInformation.numBitsDiffering(versionBits, targetVersion);
/* 176 */       if (bitsDifference < bestDifference) {
/* 177 */         bestVersion = i + 7;
/* 178 */         bestDifference = bitsDifference;
/*     */       } 
/*     */     } 
/*     */ 
/*     */     
/* 183 */     if (bestDifference <= 3) {
/* 184 */       return getVersionForNumber(bestVersion);
/*     */     }
/*     */     
/* 187 */     return null;
/*     */   }
...
/* 195 */     int dimension = getDimensionForVersion();
/* 196 */     BitMatrix bitMatrix = new BitMatrix(dimension);
/*     */ 
/*     */     
/* 199 */     bitMatrix.setRegion(0, 0, 9, 9);
/*     */     
/* 201 */     bitMatrix.setRegion(dimension - 8, 0, 8, 9);
/*     */     
/* 203 */     bitMatrix.setRegion(0, dimension - 8, 9, 8);
/*     */ 
/*     */     
/* 206 */     int max = this.alignmentPatternCenters.length;
/* 207 */     for (int x = 0; x < max; x++) {
/* 208 */       int i = this.alignmentPatternCenters[x] - 2;
/* 209 */       for (int y = 0; y < max; y++) {
/* 210 */         if ((x != 0 || (y != 0 && y != max - 1)) && (x != max - 1 || y != 0))
/*     */         {
/*     */ 
/*     */           
/* 214 */           bitMatrix.setRegion(this.alignmentPatternCenters[y] - 2, i, 5, 5);
/*     */         }
/*     */       } 
/*     */     } 
/*     */     
/* 219 */     bitMatrix.setRegion(6, 9, 1, dimension - 17);
/*     */     
/* 221 */     bitMatrix.setRegion(9, 6, dimension - 17, 1);
/*     */     
/* 223 */     if (this.versionNumber > 6) {
/*     */       
/* 225 */       bitMatrix.setRegion(dimension - 11, 0, 3, 6);
/*     */       
/* 227 */       bitMatrix.setRegion(0, dimension - 11, 6, 3);
/*     */     } 
/*     */     
/* 230 */     return bitMatrix;
/*     */   }
/*     */ 
/*     */ 
/*     */   
...
/* 244 */       this.ecCodewordsPerBlock = ecCodewordsPerBlock;
/* 245 */       this.ecBlocks = new Version.ECB[] { ecBlocks };
/*     */     }
/*     */     
/*     */     ECBlocks(int ecCodewordsPerBlock, Version.ECB ecBlocks1, Version.ECB ecBlocks2) {
/* 249 */       this.ecCodewordsPerBlock = ecCodewordsPerBlock;
/* 250 */       this.ecBlocks = new Version.ECB[] { ecBlocks1, ecBlocks2 };
/*     */     }
/*     */ 
/*     */ 
/*     */ 
...
/* 257 */       return this.ecCodewordsPerBlock;
/*     */     }
/*     */     
/*     */     public int getNumBlocks() {
/* 261 */       int total = 0;
/* 262 */       for (int i = 0; i < this.ecBlocks.length; i++) {
/* 263 */         total += this.ecBlocks[i].getCount();
/*     */       }
/* 265 */       return total;
/*     */     }
/*     */ 
...
/* 272 */       return this.ecCodewordsPerBlock * getNumBlocks();
/*     */     }
/*     */     
/*     */     public Version.ECB[] getECBlocks() {
/* 276 */       return this.ecBlocks;
/*     */     }
/*     */   }
/*     */ 
/*     */ 
...
/* 291 */       this.dataCodewords = dataCodewords;
/*     */     }
/*     */     
/*     */     public int getCount() {
/* 295 */       return this.count;
/*     */     }
/*     */     
/*     */     public int getDataCodewords() {
/* 299 */       return this.dataCodewords;
/*     */     }
/*     */   }
/*     */ 
/*     */ 
...
/* 307 */     return Integer.toString(this.versionNumber);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
